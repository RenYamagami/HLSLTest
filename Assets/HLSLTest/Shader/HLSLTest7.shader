...
return lerp(a.x, a.y, fp.x);
            ...
            float scale =0.69;
				...
				float uvX = (i.uv.x-0.5)*cos(180)+(i.uv.y-0.5)*sin(180);
				float uvY = -(i.uv.x-0.5)*sin(180)+(i.uv.y-0.5)*cos(180);
                ...
                // apply fog
                float2 p = i.uv;
				float indexX = floor((rand(float3(i.uv.x,i.uv.y,1))-float2(0.5,0.5))*lerp(0,0.3,sin(s)));
				float indexY = floor((rand(float3(i.uv.x,i.uv.y,1))-float2(0.5,0.5))*0.1)*-abs(sin(s));
